...
function Villa() {
  ...
  const glass = useMemo(() => new THREE.MeshPhysicalMaterial({
    color: "#dfe7e8", metalness: 0.08, roughness: 0.08, transmission: 0.55, thickness: 0.35
  }), []);
  useFrame((_, delta) => {
  ...
  return (
    <group ref={group}>
      ...
      <mesh position={[-1.35, 0.38, 1.75]}>
        <boxGeometry args={[5.0, 0.04, 2.0]} />
        <meshStandardMaterial color="#536f78" metalness={0.1} roughness={0.15} />
      </mesh>
      <mesh position={[-1.2, 0.5, 2.35]}>
        <boxGeometry args={[4.9, 0.08, 0.08]} />
        <meshStandardMaterial color="#8f8778" metalness={0.65} roughness={0.28} />
      </mesh>
    </group>
  );
}
...
function Scene() {
  return (
    <>
      ...
      <Float speed={0.45} rotationIntensity={0.03} floatIntensity={0.08}>
        <Villa />
      </Float>
  ...
  return (
    <main className="sceneai-test">
      ...
      <section className="sceneai-copy sceneai-copy--hero">
        <p>ARCHITECTURE / 01</p><h1>THE<br/><em>RESIDENCE</em></h1><span>Scroll to enter</span></section>
      <section className="sceneai-copy sceneai-copy--middle">
        <p>CRAFTED FOR LIVING</p><h2>LIGHT<br/>MEETS FORM</h2><span>Material · Space · Silence</span></section>
      <section className="sceneai-copy sceneai-copy--end">
        <p>PRIVATE RESIDENCE · DUBAI</p><h2>AN<br/>ARCHITECTURAL<br/><em>EXPERIENCE</em></h2></section>
    </main>
  );
}
